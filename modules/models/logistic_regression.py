...
def logistic_regression_trainer() -> None:
    ...
    vectorizer = TfidfVectorizer(
        max_features=5000, ngram_range=(1, 3), sublinear_tf=True, min_df=5, max_df=0.8
    )

    for aspect in ["appearance", "aroma", "palate", "taste"]:
        train_logistic_model(df, aspect, vectorizer)

    vectorizer_path = f"{MODELS_DIRECTORY}/logistic_regression_vectorizer.pkl"
    joblib.dump(vectorizer, vectorizer_path)


def train_logistic_model(
    df: pd.DataFrame, aspect: str, vectorizer: TfidfVectorizer, is_sample=False
) -> None:
    ...
    df_balanced = pd.concat(
        [
            resample(
                df_aspect[df_aspect[f"{aspect}_sentiment"] == label],
                replace=True,
                n_samples=max_size,
                random_state=42,
            )
    ...
    model_path = f"{MODELS_DIRECTORY}/{aspect}_logistic_regression_model.pkl"
